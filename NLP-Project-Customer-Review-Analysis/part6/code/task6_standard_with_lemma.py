...
stopwordsFile="stopwords_4000.txt"
   ...
   with open(path+ftext,'r') as f:
      for line in f.readlines():
          ...
          for word, tag in tagged:
              ...
              if wordnetTag is None:  # not supply tag in case of None
                  lemma = lemmatizer.lemmatize(word)
              else:
                  lemma = lemmatizer.lemmatize(word, wordnetTag)
              mywords.append(lemma)
          newline = " ".join(mywords)
          txt.append(newline)
          idx += 1
          if idx % 100 == 0:
              print(idx)
...
def stopwords():
    ...
    with io.open(path + stopwordsFile, 'r', encoding='utf-8') as f:
        for line in f.readlines():
            ...
            #    else:
            #        lemma = lemmatizer.lemmatize(word, wordnetTag)
            lemma = lemmatizer.lemmatize(line)
            if lemma == 'a' or len(lemma) > 1:
                sw.append(lemma)
...
def clfNB(data, labels, stwords):
   ...
   trainX, testX, trainY, testY = train_test_split(trainD1, labels, test_size = 0.25, random_state = 42)
   clf = Pipeline([('vect', CountVectorizer(min_df=1, max_df=0.45, ngram_range=(1, 5))),  # lowercase=True by default
...
if __name__ == '__main__':
   ...
   clfNB(fullData, labels, my_stopwords)
