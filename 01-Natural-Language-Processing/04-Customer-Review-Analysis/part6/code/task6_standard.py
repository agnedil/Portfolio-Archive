...
def clfNB(data, labels, stwords):
   ...
   clf = Pipeline([('vect', CountVectorizer(analyzer='word', stop_words=stwords, min_df=15, max_df=0.25, ngram_range=(1, 5))),  # lowercase=True by default
...
def clfSGD(data,labels, stwords):
   ...
   trainX, testX, trainY, testY = train_test_split(trainD1, labels, test_size=0.2, random_state=42)
   clf = Pipeline([('vect', CountVectorizer(analyzer='word', stop_words=stwords, min_df=15, max_df=0.26, ngram_range=(1, 2))),    # initially min_df=15, max_df=0.22
...
def clfRF(data, labels, stwords):
   ...
   trainX, testX, trainY, testY = train_test_split(trainD1, labels, test_size=0.2, random_state=42)
   clf = Pipeline([('vect', CountVectorizer(analyzer='word', stop_words=stwords, min_df=15, max_df=0.22, ngram_range=(1, 6))),
                   ('tfidf', TfidfTransformer()),
