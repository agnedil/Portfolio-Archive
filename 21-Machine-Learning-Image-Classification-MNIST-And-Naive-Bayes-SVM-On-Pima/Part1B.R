...
#replace zeroes with NAs in attributes 3, 4, 6, 8
nbfeaturesx<-featuresx
for (i in c(3, 4, 6, 8))
{
  zeroToRepl<-featuresx[, i]==0
  nbfeaturesx[zeroToRepl, i]=NA
}
...
#Naive Bayes with averaging over 10 test-train splits and using examples with NAs both in training and testing
for (i in 1:10) {
  
  #create training and testing sets
  traindata<-createDataPartition(y=labelsy, p=.8, list=FALSE)           #80%/20% split
  trainfeaturesx <- nbfeaturesx[traindata, ]                            #training features
  trainlabelsy   <-   labelsy[traindata]                                #training labels (true)
  testfeaturesx  <- nbfeaturesx[-traindata, ]                           #testing features
...
#output accuracies
cat("HW1. Part 1B. Naive Bayes Results. Using NAs in Attributes 3, 4, 6, 8\n")
