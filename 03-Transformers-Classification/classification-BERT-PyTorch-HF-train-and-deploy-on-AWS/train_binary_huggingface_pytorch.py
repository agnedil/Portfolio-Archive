...
from transformers import BertForSequenceClassification, DistilBertForSequenceClassification, AlbertForSequenceClassification, AdamW, get_linear_schedule_with_warmup
import torch
...
def get_dataloader( X, y, tokenizer, batch_size, maxlen ):
    ...
    encoded_data_temp = tokenizer.batch_encode_plus( X,
    ...
    print(f"Max len input_ids:      {max([ len(i) for i in encoded_data_temp['input_ids'] ])}")
    print(f"Max len attention_mask: {max([ len(i) for i in encoded_data_temp['attention_mask'] ])}")
    del encoded_data_temp
    ...
    print(f"Max len input_ids:      {max([ len(i) for i in encoded_data['input_ids'] ])}")
    print(f"Max len attention_mask: {max([ len(i) for i in encoded_data['attention_mask'] ])}\n")
...
X_test2 = df[ df['subset'] == 'test2' ]['sentence'].values
y_test2 = df[ df['subset'] == 'test2' ]['target'].values

print( X_train.shape, y_train.shape, X_val.shape, y_val.shape, X_test.shape, y_test.shape, )

'''
# AutoTokenizer() accepts only lists
X_train = df[ df['subset'] == 'train' ]['sentence'].tolist()
y_train = df[ df['subset'] == 'train' ]['target'].tolist()
X_val   = df[ df['subset'] == 'val' ]['sentence'].tolist()
y_val   = df[ df['subset'] == 'val' ]['target'].tolist()
X_test  = df[ df['subset'] == 'test' ]['sentence'].tolist()
y_test  = df[ df['subset'] == 'test' ]['target'].tolist()
X_test2 = df[ df['subset'] == 'test2' ]['sentence'].tolist()
y_test2 = df[ df['subset'] == 'test2' ]['target'].tolist()
...
model_type     = 'albert-base-v2'                # 'distilbert-base-uncased', 'albert-base-v2', 'roberta-base'
epochs         = 10
learning_rates = [ 4e-5 ]#[ 5e-5, 4e-5, 3e-5, 2e-5 ]
batch_sizes    = [ 32, ]#[ 32, 16, 8, ]                      # first distilbert was 16
douts          = [ 0.15, ]
...
with open( file_name, 'w', encoding='utf-8' ) as f:
    ...
    for LR, batch_size, dout in all_combinations:
        ...
        dataloader_test2 = get_dataloader( X_test2, y_test2, tokenizer, batch_size, maxlen )
